-- Fix 42702 "column reference blocked_at is ambiguous" in list_my_organizations:
-- the OUT param blocked_at collided with the column in the FOR-loop WHERE.
-- Qualify the column; drop the unused variable. No behavior change otherwise.
...
BEGIN
  ...
  FOR v_org IN
    ...
    WHERE public.organizations.blocked_at IS NULL
    ORDER BY public.organizations.created_at DESC
  LOOP
    v_role := NULL;
