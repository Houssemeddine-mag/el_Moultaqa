-- ============================================================================
-- Migration 015: Feedback Table for Presentation Ratings
-- ============================================================================
-- Adds a feedback table to every org schema for attendee presentation ratings.
--
-- Also patches:
--   create_org_schema()  — adds feedback table to new org schema creation
--   org_query()          — adds feedback to the allowed list
--   org_insert()         — adds feedback to the allowed list
-- ============================================================================

-- ============================================================================
-- 1. Patch existing org schemas — add feedback table to any already-created schemas
-- ============================================================================
DO $$
DECLARE
  r RECORD;
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Added feedback table to schema: %', r.schema_name;
  END LOOP;
END;
$$;
...
-- ============================================================================
-- 2. Patch create_org_schema() to include the feedback table for future orgs
-- ============================================================================
...
-- ============================================================================
-- 3. Patch org_query to allow querying feedback
-- ============================================================================
...
DECLARE
  ...
  v_table  TEXT;
  v_result JSONB;
  v_allowed_tables TEXT[] := ARRAY['users', 'events', 'sessions', 'speakers', 'tickets', 'notifications', 'questions', 'feedback'];
  v_public_tables  TEXT[] := ARRAY['events', 'sessions', 'speakers', 'notifications', 'questions', 'feedback'];
  v_org_clerk_id TEXT;
...
-- ============================================================================
-- 4. Patch org_insert to allow inserting feedback (attendees)
-- ============================================================================
...
BEGIN
  ...
  -- feedback: any registered attendee can insert
  IF v_table = 'feedback' THEN
    EXECUTE format(
      'SELECT EXISTS (SELECT 1 FROM %I.users WHERE clerk_user_id = $1)',
      v_schema
    ) INTO v_is_registered USING v_requesting_user;
